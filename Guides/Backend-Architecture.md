...
## 2. Multi-User Data Isolation (`DeviceID`)
...
## 3. Connecting to Azure SQL Database (`pyodbc`)
...
*   **Functionality:** Queries the database using a stored procedure (`[dbo].[GetGraphPresets]`), dynamically pulls column headers and rows, and packs them into a clean JSON list so the frontend can display user-saved graph axis label presets. It securely filters the returned presets by the provided `device_id`.

### C. Creating Label Presets
*   **Route:** `POST /create_labels`
*   **Libraries Used:** `Flask`, `pyodbc`
*   **Functionality:** Extracts incoming JSON data (`xlabel`, `xunit`, `ylabel`, `yunit`, and `device_id`) submitted via a frontend form, executes the `[dbo].[CreateGraphPreset]` stored procedure, commits the transaction, and returns a `201 Created` status. 

### D. Updating Label Presets
*   **Route:** `PUT /update_label/<int:id>`
*   **Libraries Used:** `Flask`, `pyodbc`
*   **Functionality:** Captures an ID from the URL parameter along with modified JSON form inputs, then executes the `[dbo].[UpdateGraphPreset]` stored procedure targeting that specific row ID in the database. 

### E. Deleting Label Presets
*   **Route:** `DELETE /delete_label/<int:id>`
*   **Libraries Used:** `Flask`, `pyodbc`
*   **Functionality:** Takes a preset ID from the endpoint URL and deletes the corresponding record from the `[dbo].[GraphPresets]` table via the `[dbo].[DeleteGraphPreset]` stored procedure.
...
*   **Functionality:** 
    ...
    4.  Calls the `[dbo].[ClearDeviceRawData]` stored procedure to wipe only the previous data belonging to that specific `device_id`. 
    5.  Loops through every row and column, using the parameterized `[dbo].[InsertRawData]` stored procedure to securely store the dataset layout row-by-row alongside the `device_id`.

### G. Fetching Uploaded CSV Columns
*   **Route:** `GET /csv_table`
*   **Libraries Used:** `Flask`, `pyodbc`
*   **Functionality:** Runs a distinct SQL query (`SELECT DISTINCT ColumnName FROM RawGraphData WHERE DeviceID = ?`) to extract all unique column names from the uploaded dataset, returning them to the frontend so users can select their X and Y graphing axes. The `WHERE` clause ensures users only see columns from their specific upload.

### H. Renaming Columns
*   **Route:** `PUT /rename_column`
*   **Libraries Used:** `Flask`, `pyodbc`
*   **Functionality:** Takes an `old_name`, `new_name`, and `device_id` via JSON and securely executes a SQL `UPDATE` statement to alter the underlying column name directly inside the database table schema. It utilizes an `AND DeviceID = ?` check to guarantee a column rename only impacts the user who requested it.
...
*   **Functionality:** This is the core computational engine of the application:
    1.  **Fetches Constraints:** Pulls axis choices (`x_column`, `y_columns`), optional unit presets, and `device_id` from the request and database.
    2.  **Pivots Data:** Reads raw row-value records from SQL into Pandas (strictly filtered by `DeviceID`), pivoting the tall data format back into a clean wide-format dataframe (`df_raw.pivot(...)`).

...
## 2. Centralized State & Session Management (`frontend/src/App.jsx`)
...
### CSV Data Ingestion (`frontend/src/CSVUpload.jsx`)
...
### Interactive Data Grid & Inline Editing (`frontend/src/GraphTable.jsx`)
...
### Preset Management (`frontend/src/LabelList.jsx` & `frontend/src/LabelForm.jsx`)
...
### Graph Rendering (`frontend/src/GraphGenerator.jsx`)
...
## 4. Styling & UI Organization (`frontend/src/"{any_file_name}".css`)
To keep the project structure clean and maintainable, I modularized the styling by pairing each React component with its own dedicated CSS file (e.g., `frontend/src/CSVUpload.css`, `frontend/src/GraphTable.css`). By utilizing React's `className` attribute, I scoped styles directly to their respective components, avoiding the complexity and cascading conflicts of a single, bloated global stylesheet. 
